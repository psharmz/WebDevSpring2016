(function(){
    ...
    function FieldController(FieldService, FormService, $scope, $routeParams, $uibModal) {
    	...
    	// using the $routeParams service, extract the userId and formId path params
        ...
        if ($scope.formId){
            ...
            FieldService
            	//get the fields for the userId and formId
                .getFieldsForForm($scope.formId)
                .then(retrievedFields);
        }

        // .then function ...kinda like a callback
        ...
        // implement event handler addField
        // uses Field Service to add a new field to a form 
        function addField(fieldType){
            ...
            // uses FieldService to add a new field to a form 
            FieldService
            	//get the fields for the userId and formId
                .createFieldForForm($scope.formId, field)

            //updates view with the list of fields
            $scope.fields.push(field);

        }

        // implement event handler deletField(field)
        // uses FieldService to remove the field from a form
        function deleteField(index){
        	// select the field we want to delet (using the index)
            var fieldId = $scope.fields[index]._id;
            //use the fieldService to delete field for the form 
            FieldService.deleteFieldFromForm($scope.formId, fieldId)
            //updates the view with a list of fields 
